...
#project building a combined radar-lidar level 3 data product 3S-GEOPROF-COMB.
...
def calculate_combined_vis(cpr_binary,vfm_binary):
    vfm_fillna = xr.where(~vfm_binary.notnull() & cpr_binary.notnull(),0,vfm_binary)
    cpr_fillna = xr.where(~cpr_binary.notnull() & vfm_binary.notnull(),0,cpr_binary)
...
def contour(ds,key='CPR_Cloud_mask',ax=None,cb=True,coords='bin',n=10,cmap='viridis'):
    ...
    if coords=='bin':
        nprofs,nbins = ds.sizes['profile'],ds.sizes['bin']
...
def read_convert_merge_granule(filepaths,new_folder_path,keep_only=None,
                               resources_dir=Path(os.path.expandvars('/projects/$USER/cloudsat_work/resources'))
):
    #unpack
    radar_filepath, lidar_filepath = filepaths
    
    #MAKE 'DUMMY' METHOD TO RETURN RIGHT SHAPE ALL -9s
    ...
    #handle when 2B-GEOPROF and 2B-GEOPROF-LIDAR have different DEM_elevation fields
    except xr.MergeError:
        #mask profile where discrepancy is larger than half a radar bin
        diff  = np.abs(cpr.DEM_elevation-vfm.DEM_elevation)<240
        ndiff = np.sum(diff.data)
        print(f'Conflicting DEM_elevation values for {ndiff:0d} profiles in {radar_filepath} and {lidar_filepath}')
        if ndiff>18550:
            print(f'>50% of profiles conflict, skipping granule {os.path.split(radar_filepath)[-1]} and {os.path.split(lidar_filepath)[-1]}')
            return
        else:
            print(f'<50% of profiles conflict, setting bad data quality for conflicting profiles for granule {os.path.split(radar_filepath)[-1]} and {os.path.split(lidar_filepath)[-1]}')
            #when DEM conflicts, set flag 1: 'GPS data not valid' to mask data
            dq = cpr.Data_quality.where(~diff,other=2)
            cpr['Data_quality']  = dq
            vfm['Data_quality']  = dq
            #overwrite 2B-GEOPROF-LIDAR DEM and retry merge
            vfm['DEM_elevation'] = cpr['DEM_elevation']
            ds = xr.merge([cpr,vfm],compat='no_conflicts')

    #assign do-op flag
    ds = ds.assign(doop_flag=get_doop_flag(cpr,filename=resources_dir/'doop_spldict.json'))
...
def broadcast_cloud_level_thresholds_to_binary_masks(cloud_bounds,da):
...
def get_doop_flag(ds,filename='../resources/doop_spldict.json',anomaly_start=np.datetime64('2011-04-17')):
    ...
    ii = np.arange(ds.sizes['profile'])
    
    if ds.start_time.data>=anomaly_start: #NOTE: changed for v5.1!!
        #after battery anomaly doop = 2
        doop_flag = np.zeros((ds.sizes['profile']))+1 #NOTE: changed for v5.1!!
    else:
